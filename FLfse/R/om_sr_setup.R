...
om_sr_model <- function(stk, sr_model="bevholt", method="L-BFGS-B", it, iy, fy){
  ...
  if (method == "L-BFGS-B"){
    srbh  <- fmle(as.FLSR(stk, model=sr), method = method, lower=c(1e-6, 1e-6), upper=c(max(rec(stk_om)) * 3, Inf))
    srbh0 <- fmle(as.FLSR(stk0, model=sr), method= method, lower=c(1e-6, 1e-6), upper=c(max(rec(stk_om)) * 3, Inf))
  }
